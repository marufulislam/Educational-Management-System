                    ...
                    <div class="portlet box blue ">
                        ...
                        <div class="portlet-body  flip-scroll">
                            ...
                            <table class="table table-bordered table-striped table-condensed flip-content" id="sample_1">
                                ...
                                <tbody>
                               <?php
                                    $sql_user_admin = "SELECT * FROM tbl_photo_gallery";
									$qry_user_admin = mysql_query($sql_user_admin);
									while($row = mysql_fetch_array($qry_user_admin)){
                        			$pk = $row['photo_gallery_id'];
                                ?>
                                <tr>
                                    <td><input type="checkbox" class="checkboxes" value="1" /></td>
                                    <td><?php echo $row['catagory_id']?></td>
                                     <td><?php echo $row['photo_caption']?></td>
                                     <td><a href="../images/school_photo/<?php echo $row["gallery_image_name"];?>" class="html5lightbox" title=""><img src="../images/school_photo/thumbs/small<?php echo $row["gallery_image_name"];?>" alt="<?php echo $row['gallery_image_name']?>" width="64" height="64" class="thumbnail"/></a></td> 
                ...
                <div class="tab-pane <?php if(@$_REQUEST['temp']=="Save" || @$_REQUEST['temp']=="View") {echo "active";} else{echo "";}?>" id="tab_1">
					...
					if(isset($_REQUEST['temp'])){
						if($_REQUEST['temp']=="Save" || $_REQUEST['temp']=="View"){
							...
							$sql_view = "SELECT * FROM tbl_photo_gallery WHERE photo_gallery_id='$Editid';";
                        ...
                        <div class="portlet-body form">
                            ...
                            <div class="form-body"><!--Begin form-body-->
                                <div class="form_result">
                                    ...
                                    if(isset($_REQUEST['temp'])){
                                        if($_REQUEST['temp']=="Delete"){
											...
											$query = mysql_query("SELECT * FROM tbl_photo_gallery WHERE photo_gallery_id = '".$Deleteid."'");		
											$sPhoto = mysql_result($query, 0, "image");
											
											$path1 = '../images/school_photo/'.$sPhoto;
											if(file_exists($path1) == true && $sPhoto != "") unlink($path1);
												
											$path2 = '../images/school_photo/thumbs/small'.$sPhoto;
											if(file_exists($path2) == true && $sPhoto != "") unlink($path2);
                                            
                                            $sql_delete="DELETE FROM tbl_photo_gallery WHERE photo_gallery_id='".$Deleteid."'";
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-md-9">
                                        <select id="catagory_id" class="form-control span6 required" name="catagory_id">
                                          <option value="">Select an option</option>
										   <?php 
                                            $rs = mysql_query("SELECT * FROM tbl_photo_catagory ORDER BY catagory_id ASC");
                                            while($obj = mysql_fetch_assoc($rs)) {
                                              echo "<option value = '".$obj['catagory_id']."'";
                                              if(@$rs_view['catagory_id'] == $obj['catagory_id']) echo "selected='selected'" ; 
                                                echo ">".$obj['catagory_name']."</option>";          
                                            } 
                                            ?>
                                ...
                                <!---------------------->
                             <div class="form-group">
                             <label class="control-label col-md-3">Photo Caption</label>
                             <div class="col-md-9">
                               <input type="text" id="photo_caption" class="form-control required" name="photo_caption" 
                                        value="<?php if (isset($rs_view["photo_caption"])) echo $rs_view["photo_caption"]; ?>"/>
                             ...
                             <div class="col-md-9">
                                 <?php
                                    if(@$_REQUEST['temp']=="Save"||@$_REQUEST['temp']=="View"){
                                    @$path = '../images/school_photo/'.@$rs_view["gallery_image_name"];
                                    if(@$rs_view["gallery_image_name"] != "" && file_exists($path)==true)
                                    {
                                    ?>
                                    <p>
                                    <button type="button" class="btn">Uploaded File :&nbsp;&nbsp; <strong> <?php echo $rs_view["gallery_image_name"];?></strong> </button>
                                    <img src="../images/school_photo/thumbs/small<?php echo $rs_view["gallery_image_name"];?>" width="128" height="128" />
                                    </p>
                                    <br />
                                    <?php
                                    }
