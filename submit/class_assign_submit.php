...
$class_short_form = escape(isset($_POST['class_short_form'])) ?  $_POST['class_short_form'] : '';
$num_of_subject = escape(isset($_POST['num_of_subject'])) ?  $_POST['num_of_subject'] : '';
...
if($temp == "Add"){
	...
	$qry_having = mysql_query("SELECT * FROM `tbl_class` WHERE class_name='".$class_name."'");
	if(mysql_num_rows($qry_having) == 0){
	
		$sql = "INSERT INTO `tbl_class`(`class_name`,`class_short_form`,`num_of_subject`,`status`,`school_id`,`created_on`) VALUES ('".$class_name."','".$class_short_form."','".$num_of_subject."','".$status."','".$user_id."','".$datetime."')";
...
if($temp == "Update"){
	...
	$sql_delete="DELETE FROM tbl_class WHERE class_id='".$Editid."'";
	$rs_delete=mysql_query($sql_delete); 
	
	
	
	
		$sql = "INSERT INTO `tbl_class`(`class_id`,`class_name`,`class_short_form`,`num_of_subject`,`status`,`school_id`,`created_on`) VALUES ('$Editid','".$class_name."','".$class_short_form."','".$num_of_subject."','".$status."','".$user_id."','".$datetime."')";
		$qry = mysql_query($sql) or die('Error in insertion : ' . mysql_error($sql));
