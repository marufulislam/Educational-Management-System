...
if($temp == "Add"){
		...
		$count = count($grade_from);
		$i = 0;
		foreach($grade_from as $key=>$value){
				$i++;
			$sql= "INSERT INTO `tbl_grade_assign`(`school_id`, `class_id`, `subject_id`, `grade_from`, `grade_to`, `grade`, `gpa`, `status`, `datetime`) VALUES ('".$user_id."','".$class_id."','".$subject_id."','".$value."','".$grade_to[$key]."','".$grade[$key]."','".$gpa[$key]."','".$status."','".$datetime."')";
...
if($temp == "Update"){
	...
	$rs_delete=mysql_query($sql_delete);
		...
		$sql = "INSERT INTO `tbl_grade_assign`(`grade_assign_id`, `school_id`, `subject_id`, `grade_from`, `grade_to`, `grade`, `gpa`, `status`, `datetime`) VALUES ('$Editid','".$user_id."','".$subject_id."','".$grade_from."','".$grade_to."','".$grade."','".$gpa."','".$status."','".$datetime."')";
