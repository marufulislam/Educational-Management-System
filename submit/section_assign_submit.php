...
if($temp == "Add"){
	...
	$qry_having = mysql_query("SELECT * FROM `tbl_section` WHERE class_id='".$class_name."' AND section_name='".$section_name."'");
	if(mysql_num_rows($qry_having) == 0){
	
		$sql = "INSERT INTO `tbl_section`(`class_id`, `section_name`, `school_id`, `status`, `created_on`) VALUES ('".$class_name."', '".$section_name."', '".$user_id."', '".$status."', '".$datetime."')";
...
if($temp == "Update"){
	...
	$sql_delete="DELETE FROM tbl_section WHERE section_id='".$Editid."'";
	$rs_delete=mysql_query($sql_delete); 
	
	
	
	
		$sql = "INSERT INTO `tbl_section`(`section_id`,`class_id`, `section_name`, `school_id`, `status`, `created_on`) VALUES ('$Editid','".$class_name."', '".$section_name."', '".$user_id."', '".$status."', '".$datetime."'";
		$qry = mysql_query($sql) or die('Error in insertion : ' . mysql_error($sql));
